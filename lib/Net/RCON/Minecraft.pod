...
effect. Firewall rules may need to be added, but are beyond the scope of this
manual.
...
This module will connect to the Minecraft server automatically as soon as the
first C<command()> is run. It will stay connected unless there is an error,
and will attempt to reconnect the next time C<command()> is run.

Therefore, none of the methods in this section of the manual are stricly
necessary. However, they are here for you if you need more fine-grained
control of your connection, or more diagnostic ability.

=head2 connect

B<This method is optional.> 
diagnostics.

    eval { $rcon->connect }; # $@ will be set on error

Attempt to connect to the configured host and port, and issue the
...
The response is a L<Net::RCON::Minecraft::Response> object, which is just a
fancy way to provide different handling of the Minecraft color codes:
...
Copyright 2019 Ryan J Thompson
